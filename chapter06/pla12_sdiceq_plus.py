...
class Board:
    ...
    trap=[0]*2	#DON!トラップ
    trap_on=False	#DON!トラップ発動
    no_effect=False	#DON!トラップ無効
    ...
    def show(self):	#パネル表示
        ...
        for i in range(3,6):
            print(f"{self.panel[i]}|",end="")
        if self.trap_on==True:
            print("　>>DON<< 警報！",end="")
        if self.no_effect==True:
            print("　* DON 無効化 *",end="")
        print("\n+ー+ー+ー+")
        print("|",end="")	#３行目
        for i in range(6,9):
    ...
    def don(self, select):
        point=select-1	#リストなので-1してる
        if self.trap[0]==select or self.trap[1]==select:
            self.panel[point]="！"
            if self.trap_on==True:
                print(">> DON! << ゲームオーバー！")
                return True
                ...
                if self.no_effect==True:
                    print("DON!トラップは無効化されています")
                else:
                    print(">> DON! << 次もDONになるとゲームオーバー！")
                    self.trap_on=True
        else:
            if self.trap_on==True:
                print("DON!トラップは無効化されました")
                self.no_effect=True
            self.trap_on=False
        return False
#Boardクラスここまで
...
class Game:
    ...
    def play(self,board):	#ゲームの手順繰り返し
        while True:
            ...
            if self.reserve_check(board)==True:
                self.out=True
                break
            ...
            while True:
                ...
                if board.turn(select)==True:
                    if board.don(select)==True:
                        self.out=True
                        break
                    self.is_turned=True
                    self.score=self.score+select+(select*self.is_double)
                    self.reserve-=select
                    if board.is_all_open() and self.reserve==0:
                        print("パネルを全て開きました！")
                        board.reset()
                        self.game_round+=1
                        if self.reserve>0:
    ...
    def assign(self):	#消すパネルの指定とサイコロの目の残りのチェック
        while True:
            try:
                print("どのパネルを開きますか？")
                select=int(input("(1-9で指定します。-1は終了 >>)"))
                if select<-1 or select>9 or select==0:
                    print("1から9までの数か、-1(終了)を入力してください。")
                    continue
            except ValueError:
                print("数値を入力してください。")
                continue
